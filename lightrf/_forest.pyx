...
cpdef np.ndarray _predict(object data,
    ...
    cdef:
        ...
        SIZE_t n_indices
        np.ndarray[SIZE_t, ndim=1] indice = np.empty((n_samples), dtype=np.intp)
    ...
    _apply(data, feature, threshold, children, indice)
    out += value.take(indice, axis=0, mode='clip')

    return out

cdef void _apply(const DTYPE_t [:, :] data,
                 const SIZE_t [:] feature,
                 const DTYPE_t [:] threshold,
                 const SIZE_t [:, ::1] children,
                 SIZE_t [:] out):
    """
    Find the terminal region (=leaf node ID) for each sample in ``data``.
    """
    cdef:
        SIZE_t n_samples = data.shape[0]
        SIZE_t i
    ...
    with nogil:
        for i in range(n_samples):
            # TODO: Wrap some duplicate codes on moving nodes into an inline
            #  function `cdef inline _move_node`.
            node_id = 0
