...
def _parallel_predict_unsampled(
        X,
        out,
        feature,
        threshold,
    ...
    """
    Private function used to predict samples not used by bootstrap in parallel.
    """

    unsampled_indices = _generate_unsampled_indices(
        random_state, n_samples, n_samples_bootstrap)

    p_estimator = _predict(X[unsampled_indices, :],
                           feature,
                           threshold,
                           children,
                           value)
    ...
    with lock:
        if len(out) == 1:
            ...
            for i in range(len(out)):
                out[i][unsampled_indices, :] += p_estimator[i]
...
def _parallel_build_trees(tree, forest, X, y, sample_weight, tree_idx, n_trees,
    ...
    if forest.bootstrap:
        ...
        feature, threshold, children, value = tree.fit(
            X, y, sample_weight=curr_sample_weight, check_input=False)
    else:
        feature, threshold, children, value = tree.fit(
...
class BaseForest(MultiOutputMixin, BaseEnsemble, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if n_more_estimators < 0:
            ...
            # Collect random states of trees for setting the oob_score.
            for tree in trees:
                self.random_states.append(tree.random_state)
...
class ForestClassifier(ClassifierMixin, BaseForest, metaclass=ABCMeta):
    ...
    def _set_oob_score(self, X, y):
        """
        Compute out-of-bag score in parallel."""
        ...
        predictions = [np.zeros((X.shape[0], j), dtype=np.float64)
                       for j in np.atleast_1d(self.n_classes_)]

        lock = threading.Lock()
        Parallel(n_jobs=n_jobs, verbose=self.verbose,
                 **_joblib_parallel_args(require="sharedmem"))(
            delayed(_parallel_predict_unsampled)(
                X,
                predictions,
                self.features[i],
                self.thresholds[i],
...
class RandomForestClassifier(ForestClassifier):
    ...
    def __init__(self,
        ...
        self.childrens = []
        self.values = []
        self.internals_sum = np.zeros((self.n_estimators,), dtype=np.intp)
        self.leaves_sum = np.zeros((self.n_estimators,), dtype=np.intp)
        self.random_states = []
